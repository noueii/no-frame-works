#!/usr/bin/env bash
# check_interfaces.sh — Checks that request structs in changed files implement Validate() and Permission()
#
...
for mod in $TOUCHED_MODULES; do
  ...
  for struct in $REQUEST_STRUCTS; do
    ...
    # Check Permission
    MAX_SCORE=$((MAX_SCORE + 1))
    if grep -rl "func (.*$struct) Permission()" "$MODULE_DIR" > /dev/null 2>&1; then
      SCORE=$((SCORE + 1))
    else
      FAILURE_LIST+=("$mod: $struct missing Permission()")
    fi
  done
done
